...
def gpu_worker(stack_id: str, images_stack_id: str = "images") -> StackFactory:
    ...
    managed policy attached, so the human can push a generated image to S3
    from within the instance during Phase 0's UAT.
    """

    def _deploy(inv, app, cdk_env):
        return inv._deploy_gpu_worker(  # pylint: disable=protected-access
            app, cdk_env, stack_id, images_stack_id
        )
